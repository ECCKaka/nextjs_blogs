...
export default function Blog({ blog, photo, comments }) {
	...
	return (
		<>
			...
			<Container>
				<Row>
					<h1 className="text-600">{blog.blog_title}</h1>
				</Row>
				<Divider />
				<h6>reported on {blog.date_added}</h6>
				<Divider />
				<Row>
					<Col sm={4}>
...
export async function getStaticProps({ params }) {
	...
	const res_blog = await fetch(`http://127.0.0.1:8000/api/blog/${params.id}`);
	const blog = await res_blog.json();
	const res_comments = await fetch(
		`http://127.0.0.1:8000/api/comment/?blog_id=${params.id}`
	);
